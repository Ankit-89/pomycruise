...
import DiningKeyInfoMycruise from '../index';
import data from '../data/po';

describe('<DiningKeyInfoMycruise />', () => {
    let wrapper;
    ...
    beforeEach(() => {
        ...
        wrapper = mount(
            <DiningKeyInfoMycruise {...customData} services={services} />
        );
    });
    test('Dining key has element', () => {
        expect(wrapper.find('.dining-key-info').length).toEqual(1);
    });
});
