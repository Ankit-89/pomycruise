...
import GlobalHeaderMycruise from '../index';
import data from '../data/po';

describe('<GlobalHeaderMycruise />', () => {
    let wrapper;
    ...
    beforeEach(() => {
        ...
        wrapper = mount(
            <GlobalHeaderMycruise {...customData} services={services} />,
            { attachTo: typeof document !== 'undefined' && document.body }
        );
    });

    test('Global header has one logo', () => {
        expect(wrapper.find('.nav-brand-logo').length).toEqual(1);
    });
    test('Global header has utility nav', () => {
        expect(wrapper.find('.utility-nav-container').length).toEqual(1);
    });
    test('Session timeout modal', () => {
        expect(wrapper.find('.session-modal-dialog').length).toEqual(1);
    });
});
