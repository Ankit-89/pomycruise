...
class inputField extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            doEmptyCheck:this.props.showAstrick
        };
    ...
    componentWillReceiveProps = (nextProps) => {
        ...
        ['showCustomError', 'customErrorMsg'].every((propName) =>
            this.handleStateChangeForUpdatedProps(nextProps[propName], propName)
        );
    };
    ...
    handleStateChangeForUpdatedProps = (propValue, stateName) => {
        if (
            ...
            this.state[stateName] !== propValue
        ) {
            this.setState({
                [stateName]: propValue
            });
    ...
    allowNumbersOnly = (e) => {
        ...
        if (inputKeyCode !== null) {
            ...
            if (
                isNaN(charVal) &&
                allowedFunctions.indexOf(inputKeyCode) === -1
            ) {
                e.preventDefault();
            }
        }
    ...
    getError() {
        if (
            this.state.doEmptyCheck &&// to check if empty check is requrired
            this.props.errorMsg &&
            this.props.errorMsg.empty &&
            ...
            this.inputField.value != "" &&
            this.props.errorMsg &&
            this.props.errorMsg.invalid &&
            !this.regex.test(this.inputField.value)
        ) {
            return {
                error: true,
                borderError: true,
    ...
    validate = (event) => {
        ...
        this.returnFlagValue(event)
    };

    returnFlagValue = (e) => {
        const { returnFlagValue } = this.props;
        const { showError } = this.state;
        returnFlagValue && returnFlagValue(e, showError);
    }
    ...
    hasError = () => {
        ...
        return Boolean(
            this.state.showError ||
                this.state.showBorderError ||
                this.state.showCustomError
        );
    };
    ...
    render() {
        ...
        let bordertabIndex = this.state.showBorderError
            ? { tabIndex: 0 }
            : { tabIndex: -1 };
        let labelClass = this.state.valueChanged
                ? `input-label show`
                : `input-label`,
            placeHolderValue = placeholder;

        placeholder = this.state.valueChanged
            ? ''
            : label
                ? label
                : placeHolderValue;
        ...
        return (
            <div
                className={`common-input-group input-group ${inputClass} ${
                    this.state.showBorderError || this.state.showError
                        ? 'show-error'
                        : ''
                } ${readOnly ? 'read-only' : ''}`}
            >
                ...
                <div className="input-error-wrapper">
                    <input
                        ...
                        autoComplete={
                            restrictAutoComplete &&
                            (name === 'email' ? `off` : `new-${name}`)
                        }
                    />
                    <span
                        aria-label={errorLabel}
                        role={this.state.showError && this.state.showClearInput
                            ? 'button'
                            : ''}
                        className={`error-image ${
                            this.state.showError && this.state.showClearInput
                                ? 'show-image'
                                : ''
                        }`}
                        {...bordertabIndex}
                        ref={(errorIcon) => (this.errorIcon = errorIcon)}
                        onKeyPress={(e) => this.clearInput(e)}
                        onClick={(e) => this.clearInput(e)}
                    />
                </div>
                <p
                    id={`${id}_err`}
                    className={`error-msg ${this.hasError() ? 'show-msg' : ''}`}
                >
                    {this.state.showCustomError
                        ? this.state.customErrorMsg
                        : this.state.errorMsg}
                </p>
            </div>
        );
