...
import { convertValueToVaildDecimalPoint, getConfig, callForPaymentMonitoring } from '../commons/CUK/utilities';
...
class paymentMycruise extends React.Component {
    ...
    hasToolTipLabel = (formFields, tooltipLabel, name) => {
        ...
        return hasTooltip ? (
            <div className="tooltip-wrapper">
                <div className={`select-label ${errorClass}`}>{`${
                    formFields.label
    ...
    handleSelectAddressBoxChange = (name, value, title, event) => {
        ...
        copyOfState[`billingAddress`] = `${
            housename.length !== undefined ? `${housename},` : ''
        } ${address1.length !== undefined ? address1 : ''}`;
        copyOfState[`billingAddress2`] = `${
            address2.length !== undefined ? address2 : ''
        }`;
        copyOfState[`city`] = `${
            address3.length !== undefined ? address3 : ''
        }`;
    ...
    render = () => {
        const style = { color: '#666', textAlign: 'left', fontSize: '80%', fontFamily: 'PraxisCom-Regular' };
        const { hasReadMore, active, termsAndConditionCheckBoxChecked,  termsAndConditionCheckBoxError, isSelectAddressVisible } = this.state;
        const { formFields, labels, accesibilityLabels } = this.props.attributes;
            ...
            <div className="checkoutForm" ref={(node) => (this.node = node)}>
                <div className="checkoutForm-wrap">
                    ...
                    <div className="checkoutForm-price">
                        ...
                        <div className="checkoutForm-img" tabIndex={0} aria-label={accesibilityLabels.securedCheckoutLabel ? accesibilityLabels.securedCheckoutLabel : ''} />
                    </div>
