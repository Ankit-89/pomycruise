...
class summaryDetailsModuleMycruise extends React.Component {
    ...
    componentDidMount() {
        ...
        this.setState({
            overridePsdPaymentFromCookie: overridePsdPaymentFromCookie == "true" ? true : false
        }, () => {
    ...
    loadShoppingCart() {
        ...
        let { psdenableCC, psdenableTA, isIframeEnableCC } = paymentMycruise.attributes;
    ...
    arrangeData = (data) => {
        ...
        this.calculatePricingPerProduct(shoppingCartData);
        let tempObj = {};
        ...
        data.entries.forEach((resData) => {
            ...
            const product = {
                ...
                productID: (resData.product.productType === 'SHOREX' || resData.product.productType === 'SPA') ? resData.product.baseProduct : (resData.product.productType === 'XDINING') ? productID : resData.product.code,
                productName: (resData.product.productType === 'SPA') ? resData.product.attributes.name : resData.attributes.name,
                skuName: diningCategory !== 'SelectDining' ? (resData.product.productType === 'SPA') ? resData.product.name : resData.attributes.name : '',
                productType: (resData.product.productType === 'SHOREX' || resData.product.productType === 'XDINING' || resData.product.productType === 'SPA') ? resData.product.productType : "aibeverage",
                ...
                diningMealPeriod: (resData.product.productType === 'XDINING') ? resData.attributes.mealPeriod : '',
                diningCategory: (resData.product.productType === 'XDINING') ? diningCategory : '',
                spaTreatmentType: (resData.product.productType === 'SPA') ? resData.attributes.name : '', /// yet to validated
                spaDuration: (resData.product.productType === 'SPA') ? resData.attributes.treatmentDuration : '',
                quantity: (resData.product.productType === 'XDINING') ? resData.noOfGuests : resData.quantity ? parseInt(resData.quantity) : '',
                unitPrice_GBP: convertValueToVaildDecimalPoint(resData.basePrice.value),
                unitSalePrice_GBP: convertValueToVaildDecimalPoint(resData.totalPrice.value),
                unitPrice_local: convertValueToVaildDecimalPoint(resData.basePrice.value),
                unitSalePrice_local: convertValueToVaildDecimalPoint(resData.totalPrice.value)
            };

            lineItemsData.push(product);
        })

        const psd2 = ((isCookieExists("use_crn_psd_pmt") && this.state.overridePsdPaymentFromCookie) || (!isCookieExists("use_crn_psd_pmt") && this.state.isPSDEnabled)) ? true : false;
        ...
        analytics.setAdditionalPageTrackAttributes({
            ...
            psd2: psd2,
            paymentFailRedirect: paymentFailRedirect ? paymentFailRedirect : false,

            myCruiseBasket: {
                orderSubtotal_GBP: data.subTotal ? convertValueToVaildDecimalPoint(data.subTotal.value) : '',
                orderTotal_GBP: data.totalPrice ? convertValueToVaildDecimalPoint(data.totalPrice.value) : '',
                loyaltyDiscount_GBP: (data.totalLoyaltyDiscount) ? convertValueToVaildDecimalPoint(data.totalLoyaltyDiscount.value) : '',
                orderSubtotal_local: data.subTotal ? convertValueToVaildDecimalPoint(data.subTotal.value) : '',
                orderTotal_local: data.totalPrice ? convertValueToVaildDecimalPoint(data.totalPrice.value) : '',
                loyaltyDiscount_local: (data.totalLoyaltyDiscount) ? convertValueToVaildDecimalPoint(data.totalLoyaltyDiscount.value) : '',
                lineItems: lineItemsData
            },
            orderTotal_GBP: data.totalPriceWithTax ? convertValueToVaildDecimalPoint(data.totalPriceWithTax.value) : '',
            orderTotal_local: data.totalPriceWithTax ? convertValueToVaildDecimalPoint(data.totalPriceWithTax.value) : ''
        });
    };
    ...
    calculatePricingPerProduct = (data) => {
        data.forEach((item) => {
            let productSubtotalPrice = 0;
            let productTotalPrice = 0;

            item.forEach((passenger) => {
                const { basePrice, quantity, totalPrice, noOfGuests } = passenger;
                const subtotalmultiply = (noOfGuests) ? noOfGuests : quantity;
                productSubtotalPrice += basePrice.value * subtotalmultiply;
                productTotalPrice += totalPrice.value;
            });

            let productLoyalityDiscountAmount =
                productSubtotalPrice - productTotalPrice;

            item.pricing = {
                subtotalPrice: productSubtotalPrice,
                totalPrice: productTotalPrice,
                loyalityDiscount: productLoyalityDiscountAmount
            };
        });
    };
    ...
    cancelPayment = () => {
        ...
        let continueShopping = typeof document !== 'undefined' && document.getElementsByClassName(
            'continueShopping'
        );
                ...
                {totalItemsNum > 0 && (
                    <div>
                        ...
                        {orderCanProceed &&
                            ...
                            ((isCookieExists("use_crn_psd_pmt") && this.state.overridePsdPaymentFromCookie) ||
                            ...
                            && (
                                <PaymentMycruise
                                    ...
                                    total={totalPriceWithTax}
                                    zeroValueLabel={labels.zeroValueProductInCartLabel}
