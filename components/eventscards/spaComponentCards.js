...
class spaComponentCards extends Component {
    constructor(props) {
        super(props);
        console.log('check card type', props);
        this.state = {
            active: false,
            readMore: false,
            longText: '',
    ...
    handleReadMore = (e) => {
        ...
        this.setState((prevState) => {
            ...
            if (active) {
                ...
                innerHTML = innerHTML.substr(
                    0,
                    Math.min(innerHTML.length, innerHTML.lastIndexOf(' '))
                );
            } else {
                innerHTML = longText;
            }
    ...
    render() {
        ...
        const { name, instances, code, fromPrice } = product;
        ...
        this.props.product.instances.forEach((element) => {
            if (element.appointments) {
                element.appointments[0].appointment.forEach((value) => {
                    const newTreatementPrice = parseInt(value.treatementPrice);
                    minimumPrice = minimumPrice
                        ? minimumPrice
                        : newTreatementPrice > 0
                            ? newTreatementPrice
                            : minimumPrice;

                    if (
                        newTreatementPrice > 0 &&
                        newTreatementPrice < minimumPrice
                    ) {
                    ...
                    <div className="spaComponent__description">
                        ...
                        <div
                            ...
                            dangerouslySetInnerHTML={{
                                __html: instances[0].description
                            }}
                        />

                        {!available && (
                            <div style={{ color: 'red' }}>{notAvlLabel}</div>
                        )}
...
export default spaComponentCards;
