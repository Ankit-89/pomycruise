...
class diningEventForm extends React.Component {
    constructor(props) {
        super(props);
    ...
    createConfiguratorData() {
        ...
        SessionStorage.setItem('diningEventType', type);
        const maxGuests =
    ...
    buildEvents = (guestsObject, product, index) => {

        guestsObject.product = {
            code: product.events[index].code,
            name: product.events[index].name
        };
        const instance = product.instances.sort((a, b) => (a.timeSlot > b.timeSlot) ? 1 : ((b.timeSlot > a.timeSlot) ? -1 : 0));
        guestsObject = instance.reduce(
    ...
    rebuildGuests = (guestsObject, product) => {
        ...
        for (let i = 0; i < product.events.length; i++) {
            guestsObject = this.buildEvents(guestsObject, product, i);
        }
        // this.buildEvents(guestsObject,product,product.events.length );

        // guestsObject.product = {
        //     code: product.events[0].code,
        //     name: product.events[0].code
        // };
    ...
    rebuildInstances = (guestsObject, instance) => {
        ...
        const type = SessionStorage.getItem('diningEventType');

        if (!outOfStock && product.code === instance.associatedEventCode) {
            ...
            if (type === 'ENTERTAINMENT') {
                ...
                disabled = (tableInfo.tableSize < guestsObject.guestsNumber) || (tableInfo.minCover > guestsObject.guestsNumber);
            }
            else if (type === 'COOKERY') {
                disabled = (stock.stockLevel < guestsObject.guestsNumber);
            }

            if (!disabled && !guestsObject.products[product.code]) {
            ...
            if (!disabled && !guestsObject.timeSlots[timeSlot]) {
            ...
            if (!disabled && !guestsObject.products[product.code].timeSlots[timeSlot]) {
            ...
            if (!disabled && !guestsObject.timeSlots[timeSlot].products[product.code]) {
...
diningEventForm.defaultProps = {
    maxGuestsCookery: 12,
    maxGuestsLimelight: 6
};

export default diningEventForm;
