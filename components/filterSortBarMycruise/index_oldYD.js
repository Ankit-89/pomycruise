...
import { scrollToTop } from '../commons/CUK/utilities';
...
class filterSortBarMycruise extends React.Component {
    ...
    filterResults = () => {
        ...
        let newQuery = '';
        let newDates = '';
        if (query.length > 0 || query === false || dates.length > 0 || !dates) {
    ...
    resolveAPIData = (response, isFirstLoad) => {
        const { results = 0, products, facets, sorts, breadcrumbs } = response;
        let newState = {};
    ...
    closeAllAccordion = () => {
        let elementsOpen = document.getElementsByClassName('open item');
